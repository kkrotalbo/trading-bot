...
Estrategia (parámetros optimizados):
  LONG  : RSI(14) entre 20 y 35  +  EMA10 < EMA55
  CIERRE LONG  : RSI ≥ 65  |  Take Profit 7%  |  Stop Loss 0.5%
  SHORT : EMA10 cruza hacia abajo EMA55
  CIERRE SHORT : RSI < 35  |  Stop Loss 0.5%
...
{Style.BRIGHT}{'═'*65}
  ...
  SL: {SL_PCT*100:.1f}%  TP: {TP_PCT*100:.0f}%  RSI_L≤{RSI_LONG_MAX}  ExL≥{RSI_EXIT_LONG}  ExS<{RSI_EXIT_SHORT}
{'═'*65}{Style.RESET_ALL}""")
...
class Cuenta:
    ...
    def abrir(self, tipo: str, precio: float, rsi: float,
        ...
        insert_operacion(
            ...
            razon=f"RSI={rsi:.1f} EMA10{'<' if tipo=='LONG' else ' cruzó bajo '}EMA55",
        )
...
def main():
    ...
    while _running:
        ...
        if not cuenta.en_posicion:
            ...
            # LONG: RSI entre 20-35 y EMA10 < EMA55
